...
**Layer 3 - 假完成守卫：** 模型说"做完了"但 PostToolUse 钩子没捕获到文件写入 → 不打勾（不依赖 git）。连续 3 轮零产出标阻塞，跳下一任务。另有 git 依赖的兜底：有 git 的项目如果全程零 commit，标记疑假完成等人来看；无 git 的项目跳过这个检查（PostToolUse 已兜底）。
